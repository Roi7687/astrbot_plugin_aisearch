...
- **答案等待**：发送前记录助手回复块数量为基线 → 等待块数超过基线定位「新回复」（**180 秒**，慢速网络+深度思考足够；期间每 5 秒检查输入框是否清空，未清空补点发送按钮，连续 3 次失败报「消息发送失败」；超时前页面文本留档）→ 对新回复做文本稳定检测（连续 9 秒无变化）**且** `_is_generation_finished` 确认页面无生成中标志（停止按钮/流式光标/思考中指示，排除回答正文与隐藏元素）才算完成（总上限 300 秒）；避免旧回复/用户消息误判与长停顿提前返回；保留引用角标清理与 HTML→Markdown 转换
